...
# 将“samples”中的元素解压缩为多个列表，并将这些列表分别存储在名为“graphs_spt”、“labels_spt”、“graph_qry”、“labels_qry”、“center_spt”、“center_qry”、“nodeidx_spt”、“nodeidx_qry”、“support_graph_idx”和“query_graph_idx”的变量中。
# 最后，该函数返回这些变量的值
def collate(samples):
...
def main():
    ...
    if len(feat.shape) == 2:
        ...
        # multiple graph retrieval：多图检索
        feat = [feat]    

    config = [('GraphConv', [feat[0].shape[1], args.hidden_dim])]

    if args.h > 1:
        # 构建一个名为config的列表，其中包含了神经网络的层次结构。
        # 如果args.h大于1，那么就会添加一个名为GraphConv的层次结构，其输入和输出维度均为args.hidden_dim。
        # 这个层次结构会被重复添加(args.h - 1)次，以构建一个具有多个GraphConv层次结构的神经网络
    ...
    for epoch in range(args.epoch):
        ...
        for step, (x_spt, y_spt, x_qry, y_qry, c_spt, c_qry, n_spt, n_qry, g_spt, g_qry) in enumerate(db):
            ...
            # 计算支持集中所有子图的节点数量之和，并将其累加到 nodes_len 变量中         
        ...
        # 果当前轮的验证准确率大于之前的最大验证准确率，则更新最大验证准确率，并将当前模型的副本保存为 model_max。
        if accs[-1] > max_acc:
            max_acc = accs[-1]
            model_max = copy.deepcopy(maml)
